...
public final class TestSuite {
    ...
    private static final String LINE_REGEX = "(null|00err|true|false|\"" + PARAM_REGEX
            + "[#\\w\\s;\\-]*\"|-?[\\d]+|-?[\\d]+\\.[\\d]+)\\s:\\s\"([\\w\\s;+-]+)\"";
     ...
     *            Class to be tested.
     * @param inputs
     *            Inputs with Command line args.
     * @param expectations
     *            Expected outputs.
     * @param logFile
     *            File to store the output of this test.
     */
    public static void testFile(final Class<?> testClass, final List<String> inputs, final List<String> expectations,
            final File logFile) {
        if (inputs != null && expectations != null && !inputs.isEmpty() && !expectations.isEmpty()) {
            try {
                ...
                if (inputs.get(0).startsWith(CMD_LINE_ARGS)) {
                    final String cmdLineArgs = inputs.get(0).replace(CMD_LINE_ARGS, "");
    ...
    private static <E> void initInOutput(final List<String> inputs, final List<String> expectations, final File logFile)
            throws IOException {
                ...
                try {
                    ...
                    while (reader.ready()) {
                        ...
                        if (nLine != null) {
                            ...
                            } else if (!nLine.matches(COMMENT_PREFIX + ".*") && !nLine.isEmpty()) {
                                lines = null;
                                break;
                            }
                        }
    ...
    private static void convert(final List<String> lines, final List<String> inputs, final List<String> expections) {
        if (lines != null) {
            // Problem with same command
            for (final String line : lines) {
                if (line != null) {
                    if (line.matches(CMD_LINE_ARGS_REGEX)) {
                        final String cmdLineArgs = line.replace("\"", "");
                        ...
                        if (match.matches() && (match.groupCount() == 2 || match.groupCount() == 3)) {
                            /*
                             * group(1) == expected output group() == input
                             */
